<?
	...
	if ($act == "insert") {
		list($id) = fetch_first_row($mysqli, "SELECT IFNULL(MAX(id), 0) + 1 FROM waiting2");
		$waiting_date = date("Y") . "-" . date("m") . "-" . date("d");
		$waiting_time = $waiting_time_h . ":" . $waiting_time_m . ":00";
		
		$query = "INSERT INTO waiting2 (id, title, waiting_date, appoint_wait_cnt, test_wait_cnt, waiting_time, regi_date) VALUES ";
		$query .= "(" . $id . ", ";
		$query .= "'" . $title . "', ";
		$query .= "'" . $waiting_date . "', ";
